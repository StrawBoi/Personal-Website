...
// Accent colors aligned with site identity
const ACCENTS = {
  ...
  acts: [
    ...
    },
    {
      ...
      subBranches: [
        ...
        // Note: third branch intentionally omitted per user request (we can add later)
      ],
    },
    {
      id: "marketing",
      label: "Marketing & Business",
      color: ACCENTS.marketing,
      subBranches: [
        {
...
function computeLayout(data, canvas) {
  ...
  data.acts.forEach((act) => {
    ...
    nodes.push({ id: act.id, type: "act", label: act.label, color: act.color, x: ax, y: ay });
    links.push({ from: data.center.id, to: act.id, color: act.color });
    ...
    act.subBranches.forEach((sub, idx) => {
      ...
      nodes.push({ id: sub.id, type: "sub", label: sub.label, color: act.color, x: sx, y: sy, actId: act.id });
      links.push({ from: act.id, to: sub.id, color: act.color });
      ...
      sub.jobs.forEach((job, jdx) => {
        ...
        links.push({ from: sub.id, to: job.id, color: act.color });
      });
    });
  });
...
function curvedPath(from, to) {
  // Compute a smooth cubic curve using mid control points
...
function Node({ node, isFocused, onFocus }) {
  ...
  const common = "px-3 py-2 rounded-full border backdrop-blur-sm";
  const glow = isFocused ? `shadow-[0_0_0_2px_rgba(255,255,255,0.12)]` : "";
  const colorRing = node.color ? `border-[${node.color}]` : "border-white/20";
  ...
  const ringStyle = { boxShadow: `0 0 16px 0 ${node.color ?? "rgba(255,255,255,0.1)"}40` };

  return (
    <div
      className={`${base} ${common} ${glow} ${colorRing}`}
      style={{ ...style, borderColor: node.color ?? "rgba(255,255,255,0.18)", background: "rgba(17,17,17,0.5)", ...ringStyle }}
      onClick={(e) => {
        e.stopPropagation();
        onFocus?.(node);
      }}
...
export default function JourneyMindMap() {
  ...
  // Focus selection
  const [focus, setFocus] = useState(null); // {id, type}
  ...
  // Render paths using the same transform
    ...
    return links.map((l, i) => {
      ...
      return (
        <path
          key={`link-${i}`}
          d={d}
          stroke={l.color || "rgba(255,255,255,0.75)"}
          strokeWidth={2}
          fill="none"
        ...
        >
          ...
          {focus.type === "job" ? (
            ...
            <div>
              ...
              <div className="text-gray-400 text-sm">{focus.actId ? journeyData.acts.find(a => a.id === focus.actId)?.label : ""}</div>
            ...
            <div>
              ...
              <ul className="mt-2 space-y-1 text-gray-200 text-sm">
                {journeyData.acts
                  ...
                  ?.subBranches.map((s) => (
                    <li key={s.id}>{s.label} ({s.jobs.length} job{ s.jobs.length !== 1 ? 's' : ''})</li>
                  ))}
              </ul>
            </div>
          ) : (
            <div>
              ...
              <div className="mt-2 text-gray-300 text-sm">Click any node to focus. Use mouse wheel to zoom; drag to pan.</div>
            </div>
          )}
        </motion.aside>
      )}
  ...
  return (
    <section className="relative h-[120vh] bg-black" data-testid="journey-mindmap-section">
      <div className="absolute inset-0">
        <div className="absolute left-0 right-0 top-0 h-24 bg-gradient-to-b from-black to-transparent pointer-events-none" />
        <div className="absolute left-0 right-0 bottom-0 h-24 bg-gradient-to-t from-black to-transparent pointer-events-none" />
      </div>
      ...
      <div className="absolute top-10 left-1/2 -translate-x-1/2 text-center">
        ...
        <p className="text-gray-400 text-sm mt-1">Drag to pan • Scroll to zoom • Click nodes for details</p>
      </div>
